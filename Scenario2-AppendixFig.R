...
b2 <- b %>%
  mutate(
    ...
    sde = mu11*(1-pi01)*pL + mu10*(1-pi00)*(1-pL) - mu01*(1-pi01)*pL - mu00*(1-pi00)*(1-pL),
    sign = cde*sde,
    ratio = sde/cde,
    dif = sde - cde,
    tcde = mu111*g11 + mu110*g10 + mu101*g01 + mu100*g00 
      - (mu011*g11 + mu010*g10 + mu001*g01 + mu000*g00),
    tsde = mu111*(1-pi011)*g11 + mu110*(1-pi010)*g10 + mu101*(1-pi001)*g01 + mu100*(1-pi000)*g00
      - (mu011*(1-pi011)*g11 + mu010*(1-pi010)*g10 + mu001*(1-pi001)*g01 + mu000*(1-pi000)*g00),
    bias_cde = cde - tcde,
    bias_sde = sde - tsde,
    ...
    bias_sde_1 = (mu11*(1-pi01)*pL + mu10*(1-pi00)*(1-pL)) -
        (mu111*(1-pi011)*(g11) + mu110*(1-pi010)*(g10) + mu101*(1-pi001)*(g01) + mu100*(1-pi000)*(g00)),
    bias_sde_0 = (mu01*(1-pi01)*pL + mu00*(1-pi00)*(1-pL)) -
        (mu011*(1-pi011)*(g11) + mu010*(1-pi010)*(g10) + mu001*(1-pi001)*(g01) + mu000*(1-pi000)*(g00))
)
...
g2 <- ggplot(b5, aes(type, bias))+
  ...
  scale_x_discrete(labels=c(parse(text = TeX('$\\tilde{\\psi}-\\psi$')),
                            parse(text = TeX('$\\tilde{\\gamma}(a_D=0)-\\gamma(a_D=0)$')),
                            parse(text = TeX('$\\tilde{Pr}(Y^{a=1, d=0}=1)-Pr(Y^{a=1, d=0}=1)$')),
                            parse(text = TeX('$\\tilde{Pr}(Y^{a=0, d=0}=1)-Pr(Y^{a=0, d=0}=1)$')),
                            parse(text = TeX('$\\tilde{Pr}(Y^{a_Y=1, a_D=0}=1)-Pr(Y^{a_Y=1, a_D=0}=1)$')),
                            parse(text = TeX('$\\tilde{Pr}(Y^{a_Y=0, a_D=0}=1)-Pr(Y^{a_Y=0, a_D=0}=1)$'))))                            
...
g2
ggsave("AFig15_U_nonrare.png", dpi=300, width = 27, height = 13, units = "cm")
